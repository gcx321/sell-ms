    ...
    <div class="add">
      <el-form label-width="80px" :data="formData">
        ...
        <el-form-item label="包装费" v-show="!flag">
          <el-input-number :min="0" :max="10" v-model="formData.packingexpense"></el-input-number>
        </el-form-item>
        ...
        <!-- 提交与取消按钮 -->
        <el-form-item>
          <el-button type="primary" @click="submitEditGoodsForm">保存</el-button>
...
export default {
  ...
  methods: {
    ...
    async submitEditGoodsForm() {
      ...
      let id = JSON.parse(window.sessionStorage.getItem("goodsdetail")).id;
  ...
  created() {
    // 获取本地存储的数据
    let goods = JSON.parse(window.sessionStorage.getItem("goodsdetail"));
    this.formData.goodsname = goods.goodsname;
    this.formData.goodscategory = goods.goodscategory;
    this.formData.goodsfeature = JSON.parse(goods.goodsfeature);
    this.formData.imgUrl = goods.imgUrl;
    this.formData.goodsdesc = goods.goodsdesc;
    ...
    if (standard[0].goodsstandard === "单规格") {
      ...
      this.formData.packingexpense = standard[0].packingexpense; // 包装费
      this.formData.goodsPrice = standard[0].goodsPrice; // 价格
    } else {
      // 多规格
      this.formData.goodsstandard = "多规格";
      this.formData.mulitipleDataTab = standard; // 直接给表单赋值
    }
  }
};
</script>
